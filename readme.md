...
## 7月30日更新日志
1. 老师考勤管理基本功能全部完成
2. 员工信息管理新增兼职员工新建界面，使信息填充更为简洁
3. 兼职员工信息查看界面与全职员工界面分开
4. 排班表录入代码重构，根据排班表有效时间计算请假时长，录入应上下班数据
5. 兼容兼职助教考勤表导入
6. 可以分员工类别进行考勤导出
7. 外教年假在每年入职日更新，调整外教完整年假时长为80小时
8. 支持在员工考勤信息表中补增请假加班数据，但暂不支持多日请假的补增
9. 漏录员工或者两个表上都存在的员工可以通过上传新的原始考勤表录入新数据
10. 请假、加班、代课、换课数据全部支持导出
...
<br> **7.29 更新**
<br>- 考勤信息可以根据员工类别分别导出
<br>- 如果请假或者加班记录未在考勤导入前填写，可以在考勤界面或者请假加班界面补增，但暂不支持多日请假的补增（会在之后版本中更新）
...
<br>- 目前暂不支持修改学期跨度，将在后续版本中更新。所以新增学期时请务必确认学期起止时间填写无误。
...
## 后续开发
-   ~~6月21日开始将对特殊员工信息录入以及相关算法进行完善（完成）~~
-   ~~生成的数据表单导出功能（完成）~~
-   ~~直接导入员工一周排班表~~
-   ~~7月份将开始对老师上课考勤板块进行开发（完成）~~
-   考勤记录中支持多日请假的补增
-   允许调整学期时长
-   ~~导出加班、请假、换课、代课信息（完成）~~
-   ~~支持遗漏录入的数据在当月考勤录入完成后通过重新上传考勤原始表的方式补录（完成）~~
-   展示历史排班信息
